...
| P7-69 | P2 | fixed | **The CCPI is running on two of four pillars, and the only signal is a number nobody reads.** Measured on staging 2026-08-14: Valuation `scoredMax=0`, Risk Appetite `scoredMax=24` — both under `MIN_SCORED_MAX`, both dropped, so the headline 14 is computed over 55% of the pillar weight. The scoring is behaving exactly as designed; the causes are operational and named in the payload itself: `apify: live=false, source="baseline-no-token"` (APIFY_API_TOKEN unset — kills spxPE 18, spxPS 12 and equityRiskPremium 10 by derivation) and `buffett/putCall/aaii: live=false, source="groq"` (the three ScrapingBee scrapes fell through to the LLM — 16 more in Valuation, 55 in Risk Appetite). **What is missing is not honesty but ALERTING**: nothing fails, nothing warns, and `certainty: 56` is the whole signal. P6-35 predicted Valuation at 40 with everything up and 28 on an FMP hiccup; the real figure is 0, because it never modelled two providers down at once. **FIXED 2026-09-08 — both halves.** ALERTING: the nightly `/api/cron/ccpi-health` (Plan A) computes the CCPI with the same scoring functions, stores `calc:ccpi` + `calc:ccpi_certainty`, and emails ADMIN_EMAIL when a pillar drops below the floor, the regime worsens, or certainty <60 — naming the failed feeds via `pillar.excluded`. COVERAGE: an authenticated prod run 2026-09-08 measured **certainty 90, zero dropped pillars, all four scoring** (score 40, "Elevated Risk"). The 55%/certainty-56 reading was a 2026-08-14 snapshot BEFORE the free multpl→FMP valuation source (P7-75) and the Polygon put/call primary (2026-09-05) landed; both restored the coverage the old measurement lacked. Verified live, not just recorded. |
...
| P8-2 | P2 | fixed | Session role no longer fails open to admin. `verifyToken` in `lib/auth.ts` now requires an explicit `"admin"` or `"member"` role and rejects (returns null) any token whose role is neither — the legacy-`{exp}`-only shim that defaulted unknown roles to admin is removed. Safe as of 2026-09-03: legacy tokens have all expired (members shipped 08-27, `SESSION_MAX_AGE` 7 days). Header comment updated so it no longer claims the old behavior; typecheck clean. |
| P8-3 | P2 | fixed | Research Queue Phase 1+2 pre-prod review — three defects fixed. (1) The POST route ignored `saveRecommendation`'s boolean and returned 200 with the recommendation even when the DB write failed (the "found nothing vs. never looked" collapse on a money surface); it now returns 502 and marks the row `failed`. (2) `enqueueTicker` sent `shares_held`/`cost_basis` on every upsert, so a ticker-only research wiped a stored position to 0/null and the covered-call-vs-exit branch could never fire; those columns are now sent only when the caller supplies them, and research runs against the stored `row.sharesHeld`. (3) Tab copy claimed "computed from live prices and IV" while IV rank is always an estimate and the snapshot is closed-market — reworded to "current prices and the options-chain IV (IV rank is an estimate …)". Also scoped `saveRecommendation` by `owner_email` at the data layer. |
| P8-4 | P3 | open | Research Queue follow-ups deferred past the Phase 1+2 ship. Client-side `patch`/`remove`/`research` fetches in `research-tab.tsx` have no `res.ok`/timeout handling, so a network failure shows the user nothing until the next `load()`. The "Pause nightly" toggle and the `archived` status write state with no current effect (the nightly cron is Phase 3), and `listQueue` does not filter archived rows. `lib/research/store.ts` resolves the metering Supabase config, which falls back to the anon key, while its header comment claims service-role-only — correct the comment or require the service key. The one-click `<ResearchButton>` on every scanner row bills an Opus 5 call with no throttle or confirm. |
...
297 findings recorded · **260 fixed · 10 wontfix · 2 verified-ok · 25 open.**
...
| P8-3 | P2 | research-queue / data-integrity | Phase 1+2 pre-prod review: a failed `saveRecommendation` PATCH was returned to the client as 200 with the (unpersisted) recommendation; `enqueueTicker` clobbered `shares_held`/`cost_basis` to 0/null on every ticker-only enqueue, making the covered-call-vs-exit branch unreachable and silently wiping positions; tab copy asserted "live … IV" while IV rank is always an estimate. Fixed: 502 + `failed` on save failure, conditional shares/cost columns with research against the stored row, accurate copy, and an `owner_email`-scoped save. |
| P8-4 | P3 | research-queue / robustness | Deferred Phase 1+2 follow-ups: no client-side error/timeout handling on the tab's mutating fetches; the "Pause nightly"/archive controls are inert until the Phase 3 cron and archived rows are not filtered from the list; `store.ts` falls back to the anon key while its comment claims service-role-only; one-click research bills an LLM call per scanner row with no throttle. |
