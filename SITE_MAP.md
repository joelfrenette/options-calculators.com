...
> Generated by `node scripts/site-inventory.ts` on 2026-08-09. **Do not hand-edit the
> generated sections** — re-run the script. Sign-off checkboxes in §6 are hand-maintained.

**Totals:** 57 API routes · 87 components · 42 lib modules · 42 public tabs across 4 nav groups.
...
|---|---|---|---|---|---|
| `ccpi` ★ | Crash & Corrections Prediction Index | `CcpiDashboard` | `components/ccpi-dashboard.tsx` | 3190 | `/api/ccpi/executive-summary`<br>`/api/ccpi`<br>`/api/ccpi/cache`<br>`/api/ccpi/history` |
...
| `/api/admin/run-health-checks` | GET | dynamic="force-dynamic"<br>maxDuration=300 | — | NEXT_PUBLIC_BASE_URL<br>VERCEL_AUTOMATION_BYPASS_SECRET<br>VERCEL_URL | yes | `components/admin/health-check-panel.tsx`<br>`lib/api-contracts.ts` |
...
| `/api/cron/breadth` | GET | dynamic="force-dynamic"<br>maxDuration=300 | api.polygon.io | CRON_SECRET<br>POLYGON_API_KEY | yes | `lib/api-contracts.ts`<br>`app/api/admin/run-health-checks/route.ts` |
...
| `/api/cron/fred-snapshot` | GET | dynamic="force-dynamic"<br>maxDuration=300 | api.stlouisfed.org | CRON_SECRET<br>FRED_API_KEY | yes | `lib/api-contracts.ts`<br>`app/api/admin/run-health-checks/route.ts` |
...
| `/api/fomc-predictions` | GET | revalidate=900 | api.stlouisfed.org<br>query1.finance.yahoo.com | FRED_API_KEY | **no** | `components/fomc-predictions.tsx`<br>`lib/api-contracts.ts`<br>`lib/remediation.ts`<br>`app/api/admin/run-health-checks/route.ts` |
...
| api.polygon.io | `/api/admin/api-status`<br>`/api/cron/breadth`<br>`/api/earnings-calendar`<br>`/api/polygon-proxy`<br>`/api/polygon-tickers`<br>`/api/smart-money-etfs`<br>`/api/social-sentiment`<br>`/api/strategy-scanner`<br>`lib/qqq-technicals.ts`<br>`lib/scraping-bee.tsx`<br>`lib/sentiment-sources.ts` |
| api.stlouisfed.org | `/api/admin/api-status`<br>`/api/ccpi`<br>`/api/cpi-inflation`<br>`/api/cron/fred-snapshot`<br>`/api/fomc-predictions`<br>`/api/jobs-report`<br>`/api/macro-indicators`<br>`/api/panic-euphoria`<br>`components/cpi-inflation-analysis.tsx`<br>`lib/vix-term-structure.ts` |
...
| File | Lines | Over by |
|---|---|---|
| `components/ccpi-dashboard.tsx` | 3190 | 2590 |
