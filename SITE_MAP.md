...
> Generated by `node scripts/site-inventory.ts` on 2026-08-11. **Do not hand-edit the
> generated sections** — re-run the script. Sign-off checkboxes in §6 are hand-maintained.

**Totals:** 60 API routes · 87 components · 52 lib modules · 42 public tabs across 4 nav groups.
...
|---|---|---|---|---|---|
| `ccpi` ★ | Crash & Corrections Prediction Index | `CcpiDashboard` | `components/ccpi-dashboard.tsx` | 942 | `/api/ccpi/executive-summary`<br>`/api/ccpi`<br>`/api/ccpi/history` |
...
| `/api/ccpi` | GET | — | api.stlouisfed.org<br>production.dataviz.cnn.io<br>www.alphavantage.co | ALPHA_VANTAGE_API_KEY<br>FRED_API_KEY | yes | `components/ccpi-audit-admin.tsx`<br>`components/ccpi-dashboard.tsx`<br>`lib/api-contracts.ts`<br>`lib/ccpi/api.ts`<br>`lib/scraping-bee.tsx`<br>`app/api/admin/run-health-checks/route.ts`<br>`app/api/data-source-status/route.ts` |
