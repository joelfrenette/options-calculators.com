...
> Generated by `node scripts/site-inventory.ts` on 2026-08-08. **Do not hand-edit the
> generated sections** — re-run the script. Sign-off checkboxes in §6 are hand-maintained.

**Totals:** 53 API routes · 86 components · 36 lib modules · 41 public tabs across 4 nav groups.
...
| `/api/cpi-inflation` | GET | — | api.stlouisfed.org | FRED_API_KEY | **no** | `components/cpi-inflation-analysis.tsx`<br>`lib/api-contracts.ts`<br>`lib/remediation.ts`<br>`app/api/admin/run-health-checks/route.ts` |
...
| `/api/fmp-proxy` | GET | — | — | — | **no** | `lib/api-contracts.ts`<br>`lib/remediation.ts`<br>`app/api/admin/run-health-checks/route.ts` |
| `/api/fomc-predictions` | GET | — | api.stlouisfed.org<br>query1.finance.yahoo.com | FRED_API_KEY | **no** | `components/fomc-predictions.tsx`<br>`lib/api-contracts.ts`<br>`lib/remediation.ts`<br>`app/api/admin/run-health-checks/route.ts` |
...
| `/api/jobs-report` | GET | — | api.stlouisfed.org | FRED_API_KEY | yes | `components/jobs-report-dashboard.tsx`<br>`lib/api-contracts.ts`<br>`lib/remediation.ts`<br>`app/api/admin/run-health-checks/route.ts` |
...
| api.polygon.io | `/api/admin/api-status`<br>`/api/earnings-calendar`<br>`/api/polygon-proxy`<br>`/api/polygon-tickers`<br>`/api/smart-money-etfs`<br>`/api/social-sentiment`<br>`/api/strategy-scanner`<br>`lib/qqq-technicals.ts`<br>`lib/scraping-bee.tsx`<br>`lib/sentiment-sources.ts` |
| api.stlouisfed.org | `/api/admin/api-status`<br>`/api/ccpi`<br>`/api/cpi-inflation`<br>`/api/fomc-predictions`<br>`/api/jobs-report`<br>`/api/macro-indicators`<br>`/api/panic-euphoria`<br>`components/cpi-inflation-analysis.tsx`<br>`lib/vix-term-structure.ts` |
...
| resend.com | `/api/auth/reset-password`<br>`lib/remediation.ts` |
