...
// ============================================================================
// REDDIT — breadth across r/wallstreetbets, r/stocks, r/investing, r/options
...
export async function getRedditSentiment(): Promise<{
  ...
  try {
    ...
    for (const r of results) {
      ...
      const posts: any[] = r.value?.data?.children?.map((c: any) => c.data) || []
      for (const p of posts) {
        texts.push(`${p.title || ""} ${p.selftext || ""}`)
        postCount++
      }
    }

    if (postCount === 0) {
      console.log("[v0] Source (Reddit): no posts from any sub")
      return { score: -1, source: "no_posts", posts: 0 }
    }

    const { score, bullish, bearish } = keywordScore(texts)
    if (bullish + bearish === 0) {
      console.log("[v0] Source (Reddit): no directional keywords found")
      return { score: -1, source: "no_signal", posts: postCount }
    }
    console.log(`[v0] ✓ Source (Reddit): ${score}/100 (${bullish}B/${bearish}Be over ${postCount} posts)`)
    return { score, source: "reddit_multi", posts: postCount }
  } catch (err) {
