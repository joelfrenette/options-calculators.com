...
const ANALYZE_TABS: Array<{ id: string; label: string; tooltip: string; featured?: boolean }> = [
  {
    ...
    // "AI-powered crash probability model" contradicted the change that shipped
    // in P6-34: ai-estimate inputs were REMOVED from pillar scoring, so the
    // score is computed only from measured readings and renormalised over live
    // weight. An LLM writes the executive summary that narrates the score — and
    // that card says "Generated by Grok xAI" where it appears — but nothing a
    // model produced carries weight in the number itself. The site's most
    // prominent claim was describing the version of the index it had just
    // deliberately stopped being.
    tooltip:
      "Our flagship index. A crash probability model combining 30+ risk indicators across 4 pillars (momentum, risk appetite, valuation, macro) to estimate the chance of a major market decline. Only measured readings are scored — estimated inputs are excluded and the pillars renormalise over what was actually available. Helps you decide when to hedge or reduce risk.",
    featured: true,
  },
  {
        ...
        <footer className="bg-white border-t border-gray-200 py-4 mt-8">
          <div className="container mx-auto px-4 text-center">
            ...
            <p className="text-xs text-gray-400 max-w-4xl mx-auto">
              {/* Said "free AI-powered tools", then two clauses later said the
                  content is "based on algorithms, formulas, and third-party
                  data" — which is the accurate half. Most tools here are
                  arithmetic; a few genuinely call a model, and those say so
                  where they appear. */}
              This website and its free tools (including calculators, predictions, and analyses) are for
