...
export function ButterflyScanner() {
  ...
  useEffect(() => {
    ...
    const cached = loadScanFromCache<Parameters<typeof setSetups>[0]>("butterfly-scanner-cache")
    if (cached) {
      try {
  ...
  const handleRefresh = async (overrideMaxDayMove?: number) => {
    ...
    try {
      const response = await fetch(`/api/strategy-scanner?type=butterflies&maxDayMove=${effectiveMaxDayMove}`)
      ...
      if (data.butterflies && data.butterflies.length > 0) {
        ...
        localStorage.setItem(
          ...
          JSON.stringify({ data: data.butterflies, timestamp, savedAt: new Date().toISOString() }),
        )
      } else {
